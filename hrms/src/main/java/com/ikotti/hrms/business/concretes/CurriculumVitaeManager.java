...
public class CurriculumVitaeManager implements CurriculumVitaeService {
	...
	public Result add(CurriculumVitaeDto curriculumVitaeDto, int candidateId) {
		...
		curriculumVitaeDto.getCandidateAbilities().forEach(candidateAbility -> candidateAbility.setCandidate(candidate));
		candidateAbilityService.addAll(curriculumVitaeDto.getCandidateAbilities(),candidateId);

		curriculumVitaeDto.getCandidateEducations().forEach(candidateEducation -> candidateEducation.setCandidate(candidate));
		candidateEducationService.addAll(curriculumVitaeDto.getCandidateEducations(),candidateId);

		curriculumVitaeDto.getCandidateExperinces().forEach(candidateExperience -> candidateExperience.setCandidate(candidate));
		candidateExperienceService.addAll(curriculumVitaeDto.getCandidateExperinces(),candidateId);

		curriculumVitaeDto.getCandidateLanguages().forEach(candidateLanguage -> candidateLanguage.setCandidate(candidate));
		candidateLanguageService.addAll(curriculumVitaeDto.getCandidateLanguages(),candidateId);

		curriculumVitaeDto.getCandidateSocialMedias().forEach(candidateSocialMedia -> candidateSocialMedia.setCandidate(candidate));
